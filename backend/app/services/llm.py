...
class LLMClient:
    ...
    def anonymize_resume(self, resume_text: str) -> str:
        ...
        脱敏策略：
        1. 手机号、邮箱、URL、身份证、护照号 —— 精确匹配替换
        2. 地址 —— 仅匹配完整省市名称，避免单字误杀（如"市场"不会被误替换）
        3. 姓名 —— 仅在"姓名"标签上下文中脱敏，不盲目替换任意中文
        """
        # 手机号
        resume_text = re.sub(
        ...
        # 身份证号（18位）
        resume_text = re.sub(r'\b\d{17}[\dXx]\b', '[ID_NUMBER]', resume_text)
        # 护照号
        resume_text = re.sub(r'\b[A-Z]{1,2}\d{6,9}\b', '[PASSPORT]', resume_text)

        # 地址脱敏：仅匹配完整省市名称，不匹配单个"市/区/县"等字
        province_city_pattern = (
            r'(?:北京市|天津市|上海市|重庆市'
            r'|河北省|山西省|辽宁省|吉林省|黑龙江省'
            r'|江苏省|浙江省|安徽省|福建省|江西省|山东省|河南省'
            r'|湖北省|湖南省|广东省|海南省|四川省|贵州省|云南省'
            r'|陕西省|甘肃省|青海省|台湾省'
            r'|内蒙古自治区|广西壮族自治区|西藏自治区|宁夏回族自治区|新疆维吾尔自治区'
            r'|香港|澳门)'
            r'[^，。！？\n]*'  # 省市名后的详细地址部分
        )
        resume_text = re.sub(province_city_pattern, '[LOCATION]', resume_text)

        # 姓名脱敏：仅在明确的"姓名"标签上下文中替换
        # 匹配 "姓名：XXX" / "姓名:XXX" / "Name:XXX" 等模式
        name_label_pattern = (
            r'(?:姓名|名字|Name|name)'
            r'\s*[：:]\s*'
    ...
    def evaluate_resume(self, resume_text: str, job_description: str, model: str = None) -> Dict[str, Any]:
        ...
        # 隐私保护处理
        anonymized_resume = self.anonymize_resume(resume_text)
