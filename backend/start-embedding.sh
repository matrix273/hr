...
echo "🚀 启动 Qwen3-Embedding-0.6B 服务..."
echo ""
echo "📋 配置信息:"
echo "   端口: $PORT"
echo "   主机: $HOST"
...
echo "🚀 启动服务..."
if [ "$USE_UV" = true ]; then
    uv run python backend/start-embedding.py --host $HOST --port $PORT
else
    python backend/start-embedding.py --host $HOST --port $PORT
fi
